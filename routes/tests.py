...
class AllTestsCase(TestCase):
    ...
    def test_model_city_duplicate(self):
        """Shaharning dublikatini yaratishda xatolik yuzaga kelishini tekshirish"""
        city = City(name='A')
    ...
    def test_model_train_duplicate(self):
        """Poyezdning dublikatini yaratishda xatolik yuzaga kelishini tekshirish"""
    ...
    def test_model_train_train_duplicate(self):
        ...
        try:
            ...
            self.assertEqual({'__all__': ["Yo'l vaqtini o'zgartiring!"]}, e.message_dict)
            self.assertIn("Yo'l vaqtini o'zgartiring!", e.messages)
    ...
    def test_cbv_detail_routes_views(self):
        response = self.client.get(reverse('cities:detail_city', kwargs={'pk': self.city_A.id}))
        self.assertEqual(200, response.status_code)
    ...
    #     self.assertContains(response, "Yo'l vaqti siz istagandan ko'p!", 1, 200)
    ...
    def test_message_error_from_cities(self):
        ...
        response = self.client.post('/find_routes/', data)
        self.assertContains(response, "Bu shaharlar orqali marshrut mavjud emas!", 1, 200)
